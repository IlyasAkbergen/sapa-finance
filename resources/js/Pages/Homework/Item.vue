<template>
  <tr class="clickable">
    <th class="float-left" scope="row" @click.self="() => $emit('show')">
      <inertia-link :href="route('users.show', homework.user.id)" v-if="homework.user">
        {{ homework.user ? homework.user.name : 'Неизвестно' }}
      </inertia-link>
    </th>
    <td class="align-middle" @click.stop="() => $emit('show')">
      {{ homework.lesson ? homework.lesson.title : 'Неизвестно' }}
    </td>
